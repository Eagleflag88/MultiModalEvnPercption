...
pc_frame::pc_frame()
{
    std::cout << "pc_frame without id " << "is constructed" << std::endl;
}
...
pc_frame::~pc_frame()
{
    std::cout << "pc_frame with id " << id_ << " is destructed" << std::endl;
}

pc_frame::ptr pc_frame::create_frame()
{
    static unsigned long pc_frame_id = 0;
    return pc_frame::ptr( new pc_frame(pc_frame_id++) ); // ptr here is the shared pointer defined with typedef
}

//todo: undistortion
//todo: z-axis error
void pc_frame::filter()
{
    // Downsampling
...
void pc_frame::pub_odom(ros::Publisher odom_pub)
{
