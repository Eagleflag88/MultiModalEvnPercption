...
line_lbd_detect::line_lbd_detect(int numoctaves, int octaveratio) : numoctaves_(numoctaves), octaveratio_(octaveratio)
{
  BinaryDescriptor::Params line_params;
  line_params.numOfOctave_ = numoctaves_;
  line_params.reductionRatio = octaveratio_;
...
//    lsd->detect(gray_img, keylines_out, 2, numoctaves_, opts); // seems different from my LSD.  already removed boundary lines
//                                                                          //       std::cout<<"lsd edge size "<<keylines_out.size()<<std::endl;
    cv::Mat mask1 = Mat::ones(gray_img.size(), CV_8UC1);
    lsd->detect(gray_img, keylines_out, 2, 1, mask1);
  }
  else
  {
    cv::Mat mask1 = Mat::ones(gray_img.size(), CV_8UC1);
